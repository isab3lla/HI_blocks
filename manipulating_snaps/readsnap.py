...
def read_block(filename, block, parttype=-1, physical_velocities=True, arepo=0, no_masses=False, verbose=False, nall=[0,0,0,0,0,0]):
  ...
  if block=="POS ":
    ...
    dt = np.uint32
    block_num = 4
#only used for format I, when file structure is HEAD,POS,VEL,ID,HIMS
  elif block=="HIMS":              #This is only for the CMBlens project
    data_for_type[:] = True        #This is only for the CMBlens project
    dt = np.dtype(np.float32)      #This is only for the CMBlens project
    block_num = 5                  #This is only for the CMBlens project
  elif block=="MASS":
